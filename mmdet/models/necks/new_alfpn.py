...
from mmdet.models.DE import DeformConv2d
...
class DC_FeatureAlign(nn.Module):  # FaPN full version
    def __init__(self, in_nc=128, out_nc=128):
        ...
        # self.conv_dila_1 = nn.Conv2d(in_nc * 2, in_nc, kernel_size=1, dilation=1)
        # self.conv_dila_6 = nn.Conv2d(in_nc * 2, in_nc, kernel_size=1,  dilation=6)
        # self.conv_dila_12 = nn.Conv2d(in_nc * 2, in_nc, kernel_size=1,  dilation=12)
        # self.conv_dila_18 = nn.Conv2d(in_nc * 2, in_nc, kernel_size=1,  dilation=18)

        self.conv_dila_1 = nn.Conv2d(in_nc * 2, in_nc, kernel_size=1, dilation=1)
        self.conv_dila_6 = nn.Conv2d(in_nc * 2, in_nc, kernel_size=3, padding=6,dilation=6)
        self.conv_dila_12 = nn.Conv2d(in_nc * 2, in_nc, kernel_size=3, padding=12, dilation=12)
        self.conv_dila_18 = nn.Conv2d(in_nc * 2, in_nc, kernel_size=3, padding=18,dilation=18)
    ...
    def forward(self, feature_bottom, feature_top, main_path=None):
        ...
        feature_top_new = torch.cat((self.conv_dila_1(feature_top), self.conv_dila_6(feature_top)
                                     , self.conv_dila_12(feature_top),
                                     self.conv_dila_18(feature_top)), dim=1)

        # aa = self.conv_dila_1(feature_top)
        # bb = self.conv_dila_6(feature_top)
        # cc = self.conv_dila_12(feature_top)
        # dd = self.conv_dila_18(feature_top)
        # ipdb.set_trace()
        cat_feature = self.AFM(feature_bottom, self.up_module(feature_top_new))
...
class ALFPN(BaseModule):
    ...
    def forward(self, inputs):
        ...
        P2 = self.P2_DC(inputs[0], inputs[1])

        P2_final = P2 * self.attention_P2(P2)

        #P3 = self.P3_DC(inputs[1], self.P4_up_module(inputs[2]))
        P3 = self.P3_DC(inputs[1], inputs[2])
        P3_final = P3 * self.attention_P3(P3)
